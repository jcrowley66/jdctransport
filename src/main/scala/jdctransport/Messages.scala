 ...
 *  can be used to determine the progress (if populated - an Application responsibility).
 *
 ...
 **/
final case class ACK(connID:Long, msgID:Long, appData:Option[AppData]=None)

/** A NACK for an outbound message, with full Message -- sent back to the original sender only */
final case class NACK(connID:Long, failReason:Int, msg:Message)
